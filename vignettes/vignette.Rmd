...
The `DATED` package accompanies the paper "Localizing Rare-Variant Association Regions via
...
Here, we show how to apply `DATED`. 
...
```{r}
p.leaf <- Test_Leaf(snp_dat=snp_dat,thresh_val=M,covars=NULL,teststat="FET")
```
\par
In the output data frames `p.leaf`, each row links to a rare variant (SNPs), and the number of rows equals the number of rare variants (SNPs) we considered. The data frame includes 5 variables. In the data frame, variable `L1` is leaf ID; variable `pvals` is the leaf level p values; variable `Test` indicates the name of the statistical test to generate the leaf level p values (FET or score).

```{r}
str(p.leaf)
```
 \par
Finally, we use the function `DATED` to conduct dynamic and hierarchical testing based on the leaf level p values. 

```{r}
out <- DATED(struct_map=p.leaf,L=L,alpha=alpha)
```
In the output data frames `out`, each row links to a unique SNP that is detected by DATED. The variables `snpID`, `L1`, and `domainID` link to the detected SNP ID, leaf ID, and domain ID, respectively; Variable `Test` links to the name of the statistical test we applied (FET or score); Variable `pvals1` links to the leaf level p-values; Variable `Layer` indicates the number of the layer that SNP is detected; Variable `L2` and `L3` indicates the node ID that we tested on layer 2 and 3, respectively. SNPs that are detected in previous layers will have NA in the variables `L2` and `L3`.
